...
import (
	...
	"github.com/macsuite/macutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := macutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(macutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := macutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := macutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := macutil.NewBlockFromBytes(blockBytes)
	if err != nil {
