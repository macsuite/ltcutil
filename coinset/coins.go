...
func (cs *CoinSet) TotalValue() (value macutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue macutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue macutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount macutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue macutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue macutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue macutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue macutil.Amount, coins []Coin) (Coins, error) {
...
// macutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() macutil.Amount {
	return macutil.Amount(c.txOut().Value)
}
